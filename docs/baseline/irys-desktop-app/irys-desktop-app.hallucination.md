...
| Desktop UI direction | **IRYS + Fluent settings; dark break** | User-facing product name is `IRYS`. Settings use a NavigationView-like desktop composition with selectable light/dark appearance. The wide calm break view is dark-only to prevent nighttime flash and must retain the complete animated eye. Review artifacts live in `docs/brief/`; no `src-vue` implementation is approved by those files alone. |
