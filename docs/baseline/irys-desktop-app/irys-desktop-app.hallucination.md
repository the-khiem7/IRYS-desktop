...
| Desktop UI direction | **WinUI 3 / Windows 11 Fluent light** | User rejected the dark generic-dashboard prototype. Settings should use a NavigationView-like desktop composition; the break view should use a wide, calm two-column layout. Review artifacts live in `docs/brief/`; no `src-vue` implementation is approved by those files alone. |
